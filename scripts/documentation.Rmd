...
```{r sampling regime, fig.cap="Sampling regime for all individuals in minutes", fig.width=9}
...
Therefore, only observations with a rounded *timelag* of 900 seconds are processed further whilte the others are excluded..

A clear distinction between *resting* and *moving* isdone by the introduction of the variable *segment_trigger*. The appropriate level for this threshold is evaluated in a critical manner. Sample data sets from three different wild boar movements are generated and evaluated in a histogram. With a binwidth of 5, we are able to see a clear decline in the first bins of stepmean. This leads to our interpretation that wild boars rest on the left side of the histogram and move on the right side.
...
Setting the threshold seems reasonable at the point where stepmean flattens in a distinct manner (see histograms). In our samples this threshold can be anywhere between 30 to 50 meter. The *segment_trigger* is therefore set to 40
