...
As mentioned above, the majority of the recorded wild boar data has a rounded *timelag* of 900 seconds (≙ 15 minutes) between two points (Figure \@ref(fig:timelag)).
...
```{r stepmean-ueli, appendix=TRUE, fig.cap="Mean steplength if wildboar Ueli. Sample dataset from May 2016. Threshold set to 40 meter.", fig.align='center'}
...
```{r stepmean-caro, appendix=TRUE, fig.cap="Mean steplength if wildboar Ueli. Sample dataset from May 2016. Threshold set to 40 meter.", fig.align='center'}
...
```{r stepmean-frida, appendix=TRUE, fig.cap="Mean steplength if wildboar Ueli. Sample dataset from May 2016. Threshold set to 40 meter.", fig.align='center'}
...
```{r resting-sites-100, fig.align='center'}
...
```{r resting-sites-30, fig.align='center'}
...
```{r nest-wallows-100, fig.align='center'}
...
```{r nest-wallows-30, fig.align='center'}
...
The clear assignment of nests and wallows is the most challenging task. In our work we base our criterias for the assignment on a mix of analyzed movement patterns and preferred habitat use which we derived from literature. We are able to assign 16.4 % of all resting sites to nests, 4.3 % to wallows. The fact that assigned number of nests is four times higher than wallows can be explained. Wild boars spend more time with basic needs than comfort behavior like wallowing.

Since we stick to these criterias in a conservative manner

- konservative wahl der Kriterien
- NAs beschreiben
- Forschungsfragen beantwortet?


Apart from this uncertainty  we are confident that we have developed a robust method for the assignment of resting sites. Nevertheless, the developed method requires fine tuning and a validation of the results.
