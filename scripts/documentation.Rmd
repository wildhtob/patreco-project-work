...
A small proportion of all resting sites are assigned with NA. A quick traceback in out data reveals that this is clearly caused by missing field information (see *wallow_area* in table 5.2). The same number of NAs applies for *nest_summer_area* and *nest_winter_area*.
...
Another challenge of the developed method is finding an appropriate level of *segment_trigger*. Due to missing literature information we have chosen to base this decision on sample data. It can be discussed whether histograms of *stepmean* (see Figure \@ref(fig:3.3)) is the right method to evaluate this. Also the chosen level of *stepmean* (40 meter) is questionable and requires validation.
